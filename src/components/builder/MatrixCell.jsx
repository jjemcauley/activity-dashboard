...
export default function MatrixCell({ activity, row, col, errors, meta, simGroup, simColorMap, onDrop, onClear, onMove, dropPreview, registry, daySlices, allGroups, rotLabel, isSelected, onSelectionStart, onSelectionMove, onPaste, hasClipboard, isActivityAllowed, selectedActivity, onClickPlace, groupColor }) {
  ...
  return (
    <div
      ...
      className={`min-w-[88px] h-[52px] rounded-md flex items-center justify-center gap-1 transition-all relative overflow-hidden select-none border-[1.5px] border-[var(--cell-border)] bg-[var(--cell-bg)] ${activity ? 'cursor-grab' : (selectedActivity && !clickBlocked) ? 'cursor-pointer' : 'cursor-default'}`}
      ...
      {activity ? <>
        ...
        <div className="text-center px-1.5">
          ...
          <div className="flex gap-[3px] justify-center mt-0.5">
            {int && <span className="text-[8px] px-[3px] rounded-sm font-bold text-[var(--int-color)] bg-[var(--int-bg)]" style={{ '--int-color': int.color, '--int-bg': int.bg }}>{int.label}</span>}
            <span className={`text-[8px] font-mono ${valueScore >= 70 ? 'text-[#34d399]' : valueScore >= 40 ? 'text-[#fbbf24]' : 'text-[#666]'}`}>{meta?.value||'?'}</span>
          </div>
        </div>
        {(hasError || hasWarn) && <div className={`absolute top-0.5 right-[3px] text-[8px] font-[800] ${hasError ? 'text-[#dc2626]' : 'text-[#f59e0b]'}`}>{hasError ? 'X' : '!'}</div>}
      ...
      </> : <div className="text-[18px] font-light text-[var(--empty-color)]" style={{ '--empty-color': emptyColor }}>{clickBlocked || blocked ? 'X' : '+'}</div>}
    </div>
  );
}

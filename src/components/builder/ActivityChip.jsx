  ...
  return (
    <div draggable={!disabled} onDragStart={e => { if (disabled) return; e.dataTransfer.setData('text/plain', name); e.dataTransfer.effectAllowed = 'copy'; onDragStart?.(name); }}
      ...
      <div className="min-w-0 flex-1">
        <div className={`font-semibold whitespace-nowrap overflow-hidden text-ellipsis font-sans ${compact ? 'text-[11px]' : 'text-[13px]'} ${disabled ? 'text-text-faint' : 'text-text-primary'}`}>{compact ? shortName(name) : name}</div>
        {!compact && <div className="flex gap-1.5 mt-0.5 text-[11px]">
      ...
      {!compact && onRotAssign && (
        <div className="flex gap-0.5 ml-auto shrink-0" onClick={e => e.stopPropagation()}>
          <button
            ...
            className={`w-[22px] h-[22px] rounded-[3px] text-[11px] font-bold cursor-pointer flex items-center justify-center border border-[var(--btn-border)] bg-[var(--btn-bg)] text-[var(--btn-color)]`}
